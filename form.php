...
<?php
    ...
    if($conn->query("CREATE DATABASE delta")==false) //if database already exists, then check for unique id
    {
        ...
        while($check==0)
        {
            $_SESSION["id"]=(int)generate(); //unique id is sent as a session variable
            $existid=mysqli_num_rows($conn->query('SELECT id FROM userdetails WHERE id='.$_SESSION["id"]));
            if($existid==0)
                $check=1;
        }
        $conn->close();
